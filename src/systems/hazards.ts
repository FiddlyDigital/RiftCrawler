...
/**
 * Trap-hazard tiles: spike arming/firing and teleport-trap triggering.
 * Called every game tick/step by `Game` — these are hot-path methods and
 * trust the caller's state rather than re-validating on every call.
 */
export class HazardSystem {
  /** Advances every spike hazard's arm/warning/fire timer and applies fire damage. */
  static processHazards(game: Game): void {
    ...
    for (const h of spikeFirePositions) {
      ...
      if (game.player.x === h.x && game.player.y === h.y) {
        ...
        if (game.player.hp <= 0) { CombatSystem.triggerDeath(game, 'SPIKED', 'Impaled by floor spikes.'); return; }
      }
  ...
  /** Fires a teleport trap under `entity`, if one is there and not resisted. */
  static checkHazardTrigger(entity: { x: number; y: number }, game: Game, isPlayer: boolean): void {
  ...
  /** Moves `entity` to a uniformly random unoccupied floor tile. */
  static teleportEntity(entity: { x: number; y: number }, game: Game): void {
