 ...
 * passive regeneration, and stun-aura propagation. Called every game tick
 * by `Game` — these are hot-path methods and trust the caller's state
 * rather than re-validating on every tick.
 */
export class StatusEffectSystem {
  /** Ticks down every active status on the player and every monster, applying poison damage. */
  static applyStatusEffects(game: Game): void {
    ...
    for (const s of game.player.statuses) {
      if (s.type === 'poison' && !game.player.poisonImmune) {
        ...
        if (dmg > 0) {
          ...
          if (game.player.hp <= 0) { CombatSystem.triggerDeath(game, 'HERO DEFEATED', 'Succumbed to poison.'); return; }
        }
      }
    ...
    for (const m of poisonKilled) CombatSystem.killMonster(m, game);
  }
  ...
  /** Stuns every monster within the player's aura-stun radius, if any. */
  static applyAuraStun(game: Game): void {
