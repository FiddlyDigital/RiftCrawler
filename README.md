...
A mobile-first block-building-meets-roguelike hybrid. You play both halves of the board at once: steer falling shapes to build the dungeon floor, while moving your hero across the tiles you just laid down to fight, grow stronger, and descend. 

- Story based on Celtic Mythology (Tuatha Dé Danann cycle)
- Turn-based combat
- Classic block-building game-play
- Character progression via random boons (Geasa) and Ogham Brands
- Central nexus (Sidhe Mound) with stores and transient vendors, npcs and friends.
- Tons of NPCs, random interactions, omens that affect levels, and game-play variations means no run is ever the same!

Not in anyway associated with or endorsed by Blue Planet Software / The Tetris Company.
This is a free game that pays homage to RPGs and that falling-block game that we all know and love :) 
...
| **Classes** | chosen at run start | Chronomancer (time/tempo control), The Architect (shape-layer synergy, tankier), An Draoi (blood-priced magic, gains a patron pact). Fully data-driven from `classes.json` — see [Data files](#classesjson-biomesjson-patronsjson). |
| **Patron pacts** | An Draoi only, `src/data/patrons.json`, `src/pact.ts` | A deity emissary waits in the sídhe-mound waystation from floor 2 until An Draoi swears a pact with one of 2 offered deities (of the Morrígan, Manannán mac Lir, Tethra), each granting its own 3-spell kit as the run's ranged ability — at a permanent ATK toll. |
...
| **The Daily Rift** | `src/rng.ts`, `src/main.ts`, `src/storage.ts` | One seeded run a day — the same dungeon for every player, keyed off the UTC date (`hashSeed(dailySeedString())`). Locked to the default difficulty with no NG+ heat so scores are comparable; class and curse are still yours to pick. One attempt per day, tracked with a **streak** (consecutive days, plus a best-ever), and the death/victory screen produces a shareable summary. |
...
| **Fidchell** | `src/fidchell.ts`, `docs/fidchell.md` | Every 7th (non-boss) floor a Fomorian gambler bars the crossing with fidchell — "the wooden wisdom," built on the Irish 7×7 *tafl* game **brandub**. You're dealt a side at random: the **King's escape** (slip the High King to a corner dún) or the **Raiders' hunt** (surround and take the King). Turn-based, tap-driven, rook-slide moves with custodial (flanking) capture and a "weak" King. Win → gold, a boon, and a shortcut past the floor; lose → the gambler drops onto the floor as an elite to fight. Both sides are played by a depth-3 alpha-beta AI so whichever role you draw is a real contest (~60/40 King/raider self-play balance). Once **Midir** is freed he sets the same board up in the mound as a **wager** you can take as often as you can afford it: the stake (`balance.json` → `rescues.wagerStake`) is paid up front, a win returns it doubled plus a tier-2 Geis, and either result folds the board away and rebuilds the mound around you — no descent, no elite, the rest stop stays a rest stop. |
...
| **Rescues** | `src/data/rescues.json` | Souls-style: captives occasionally ride down inside a shape under Fomorian elite guard (captive + two captors on one piece). Kill every guard, then talk to free them — they beam away and join the mound for the rest of the run, lining its north wall and then its south. Eight to find, all Tuatha Dé Danann: the **Gobán Saor** shapes your next piece to order, **Bé Chuille** the Enchantress names the boss ahead, **Nuada Airgetlám** pours the king's draught from Goibniu's feast (+ATK until the next descent), **Airmed** the Herb-Wise turns gathered herbs into permanent Max HP (see below), **Eithne** — who saw a prophecy outrun her father — trades your Rift Curse for another, **Dian Cécht** sings the Well of Sláine over you (full heal + a deathward charge, one at a time), **Midir** of Brí Léith plays you at fidchell for gold (see below), and **Abcán** the Harper plays the suantraí so the next floor's monsters arrive drowsy. A line clear that swallows the captive loses them — they may ride again on a later floor. |
...
src/
  ...
  game.ts          The Game class — the run's shared state (grid, falling
                   piece, player, monsters) and the core block/floor loop
                   (spawn, lock, line-clears, gravity, descent, combat
                   orchestration). Larger self-contained subsystems are split
                   into their own classes, each composed onto Game as a field
                   and reached through thin delegating methods (see below)
  fidchell.ts      The brandub/tafl board challenge (every 7th floor)
  causewayDuel.ts  The boss-floor Causeway Duel (turn-based bridge-building)
  waystation.ts    The sídhe-mound rest floor + every mound/floor NPC-tile
                   bump verb (An Dagda, pact emissary, rescues, well, coffer,
                   seanchaí, ghost, smiths, hearth-fire, stall, …)
  bossEncounters.ts The Bres/Gorgoth finale + per-boss hooks (shards, surge)
  vendorOffers.ts  Altar / tattoo artist / Fear Dearg shop / rescue services
  spawning.ts      Monster spawning + vault/den dungeon rooms
  runSetup.ts      Start-of-run pickers: class, difficulty, heat, modifier
  saveGame.ts      Mid-run serialize / restore (the SavedRun contract)
  pact.ts          An Draoi's deity-pact ceremony
  npcEncounters.ts Wandering-NPC / seanchaí / ghost dialogs
  smithQuest.ts    Lugh's Spear questline (the three smiths)
  ...
  systems/
    ...
    abilities.ts     Ranged-ability dispatch (class kits, patron spells, Spear)
...
- **Injected ports** — `StashPort` (cross-run gold), an optional `now()` clock, and an optional `seed` for reproducible runs, all passed via `new Game(cb, { stash, now, seed })`. Defaults (`MemoryStash`, a guarded `performance.now()`, `Math.random`) mean the sim runs with **zero host wiring**.
...
**`biomes.json`** — ordered deepest-first, matched by `Biome.forFloor(floor)` (first entry whose `minFloor` the floor meets). Fields: `id`, `name`, `minFloor`, `tileRgb`, `moteColor`, `monsterHpMult`, `gravityPctBonus`, `desc`, `terrainType` (`swamp` | `sacred` | `ice`) — the special-tile effect that biome's terrain pieces lay down on lock — and an optional `terrainShapes` array of shape keys deciding *which* shapes lay it (defaults to `["S","L","J"]`; the deeper biomes add `O`/`I` for denser signature terrain). Crossing into a new one (including the starting floor-1 biome) logs its flavor line and toasts "Entering `<name>`...".
...
**`smiths.json`** — the three legendary smiths of Lugh's Spear questline, in encounter order (Luchta → Credne → Goibniu). Fields: `id`, `char` (sprite-map key), `name`, `partKey` (`shaft` | `bolts` | `head`), `partName`, `tagline`, `flavor`. Encountered via `SmithQuest.triggerEncounter` (`src/smithQuest.ts`), reusing the floor-event modal rather than a dedicated component; Goibniu's third meeting is a `special`-style reforge handled directly in that module rather than in JSON, since it swaps `player.rangedAbility` at runtime.
...
### Support files
- **`shapes.json` (each shape carries a `weight` — classics ~10, rare custom shapes ~1, `0` removes it from the natural rotation)** — the 9 piece shapes in the random draw pool: the 7 standard shapes plus two extra non-standard shapes (`Q`, `H`), each `{ matrix, color }` keyed by letter.
...
- **New biome:** add an entry to `biomes.json` with a `minFloor` and a `terrainType` (and optionally `terrainShapes` to tune how much of that terrain the biome lays); biome-specific bosses still need a code entry (see `bosses.json` note above).
