...
| **Waystations** | `src/game.ts` | Every staircase opens a choice — delve deeper, or rest in a safe sídhe mound first. The mound sits *between* floors (visiting never consumes a floor number, so bosses/smiths can't be dodged): no falling stone, no monsters, no fog — just a seanchaí telling the myth behind the run, a hearth-fire (one full heal), the Fear Dearg's stall, and the stairs onward. |
