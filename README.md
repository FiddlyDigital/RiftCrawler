...
| **Rescues** | `src/data/rescues.json` | Souls-style: captives occasionally ride down inside a tetromino under Fomorian elite guard (captive + two captors on one piece). Kill every guard, then talk to free them — they beam away and join the mound for the rest of the run: the Gobán Saor shapes your next piece to order, Fedelm the Seeress names the boss ahead, and Bricriu of the Feast serves the Champion's Portion (+ATK until the next descent). A line clear that swallows the captive loses them — they may ride again on a later floor. |
