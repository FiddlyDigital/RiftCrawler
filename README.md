...
| **Lugh's Spear questline** | `src/data/smiths.json`, `src/game.ts` | Every 3rd floor (skipping boss floors) toasts "You hear the clang of an anvil..." on entry, "The sound of anvils is getting stronger!" at 25 blocks, and embeds one of three legendary smiths (Luchta, Credne, Goibniu) as a guaranteed encounter on the 50th block dropped that floor (`balance.json`'s `smiths` block). Each grants one spear part; Goibniu's third meeting reforges it, replacing the player's ranged ability with a bolt that pierces every monster straight up their own Tetris column. |
